...
(() => {
  ...
  // src/constants.ts
  var NODE_SUFFIX, CDN_URL, PROXY_GROUPS, LOW_COST_NODE_MATCHER, LANDING_NODE_MATCHER, SUCAI_NODE_PATTERN, SUCAI_NODE_MATCHER, countriesMeta;
  var init_constants = __esm({
    "src/constants.ts"() {
      ...
      SUCAI_NODE_PATTERN = String.raw`sucai`;
      SUCAI_NODE_MATCHER = createCaseInsensitiveNodeMatcher(SUCAI_NODE_PATTERN);
      countriesMeta = {
        香港: {
          weight: 10,
  ...
  // src/proxy_groups.ts
  function isSucaiNode(nodeName) {
    return SUCAI_NODE_REGEXP.test(nodeName);
  }
  function withoutSucaiNodes(nodes) {
  ...
  function buildProxyGroups({
    ...
    const groups = [
      ...
      },
      {
        ...
        type: "select",
        proxies: hasTW && hasHK ? [PROXY_GROUPS.DIRECT, "台湾节点", "香港节点"] : defaultProxiesDirectWithoutSucai
      },
      {
        name: PROXY_GROUPS.BAHAMUT,
        icon: `${CDN_URL}/gh/Koolson/Qure@master/IconSet/Color/Bahamut.png`,
        type: "select",
        proxies: hasTW ? ["台湾节点", PROXY_GROUPS.SELECT, PROXY_GROUPS.MANUAL, PROXY_GROUPS.DIRECT] : defaultProxiesWithoutSucai
      },
      {
      ...
      },
      {
        ...
        type: "select",
        proxies: hasUS ? ["美国节点", PROXY_GROUPS.SELECT, PROXY_GROUPS.MANUAL] : defaultProxiesWithoutSucai
      },
      {
      ...
      lowCostNodesWithoutSucai.length > 0 || regexFilter ? {
        ...
        interval: 60,
        tolerance: 20,
        ...!regexFilter ? { proxies: lowCostNodesWithoutSucai } : {
          "include-all": true,
  ...
  var SUCAI_NODE_REGEXP;
  var init_proxy_groups = __esm({
    "src/proxy_groups.ts"() {
      "use strict";
      init_constants();
      SUCAI_NODE_REGEXP = new RegExp(SUCAI_NODE_PATTERN, "i");
    }
  });
  ...
  var require_main = __commonJS({
    "src/main.ts"() {
      ...
      function main(config) {
        ...
        const globalProxies = proxyGroups.map((item) => String(item.name)).filter((name) => name !== PROXY_GROUPS.SUCAI);
        proxyGroups.push({
