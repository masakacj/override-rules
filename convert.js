...
(() => {
  ...
  // src/constants.ts
  var NODE_SUFFIX, CDN_URL, PROXY_GROUPS, AUTO_SC_GROUP_REGEXP, LOW_COST_NODE_MATCHER, LANDING_NODE_MATCHER, countriesMeta;
  ...
  // src/proxy_groups.ts
  function isAutoScNode(nodeName) {
    return AUTO_SC_GROUP_REGEXP.test(nodeName);
  }
  function extractAutoScGroupName(nodeName) {
    const match = nodeName.match(AUTO_SC_GROUP_REGEXP);
    return match?.[1]?.trim() || null;
  }
  function withoutAutoScNodes(nodes, autoScNodeSet) {
    return nodes.filter((nodeName) => !autoScNodeSet.has(nodeName));
  }
  function combineExcludeFilters(...filters) {
    const validFilters = filters.filter(Boolean);
    return validFilters.length > 0 ? validFilters.join("|") : void 0;
  }
  function buildAutoScProxyGroups(allProxyNames) {
    const groupMap = /* @__PURE__ */ new Map();
    const autoScNodeSet = /* @__PURE__ */ new Set();
    ...
    const groups = Array.from(groupMap.entries()).map(([groupName, nodeNames]) => ({
      name: groupName,
    ...
    for (const country of countries) {
      ...
      if (!regexFilter) {
        ...
        const excludeFilter = combineExcludeFilters(
          landing ? LANDING_NODE_MATCHER.pattern : void 0,
  ...
  function buildProxyGroups({
    ...
    const groups = [
      ...
      },
      {
        ...
        type: "select",
        proxies: hasTW && hasHK ? [PROXY_GROUPS.DIRECT, "台湾节点", "香港节点"] : defaultProxiesDirectWithoutAutoSc
      },
      {
        name: PROXY_GROUPS.BAHAMUT,
        icon: `${CDN_URL}/gh/Koolson/Qure@master/IconSet/Color/Bahamut.png`,
        type: "select",
        proxies: hasTW ? ["台湾节点", PROXY_GROUPS.SELECT, PROXY_GROUPS.MANUAL, PROXY_GROUPS.DIRECT] : defaultProxiesWithoutAutoSc
      },
      {
      ...
      },
      {
        ...
        type: "select",
        proxies: hasUS ? ["美国节点", PROXY_GROUPS.SELECT, PROXY_GROUPS.MANUAL] : defaultProxiesWithoutAutoSc
      },
      {
      ...
      lowCostNodesWithoutAutoSc.length > 0 || regexFilter ? {
        ...
        interval: 60,
        tolerance: 20,
        ...!regexFilter ? { proxies: lowCostNodesWithoutAutoSc } : {
          "include-all": true,
  ...
  var require_main = __commonJS({
    "src/main.ts"() {
      ...
      function main(config) {
        ...
        const allProxyNames = (config.proxies ?? []).map((proxy) => String(proxy.name ?? "")).filter(Boolean);
